...
object Mul00Spec extends Properties("Mul00") with NodeSpecBase {
  ...
  property("var0 * nonvar0 reverse node1") = forAll(genV0(), genNV0(), genN1()) { (a: Var0, b: N0, c: N1) =>
    val g = Mul00(a, b).reverse(c)
    g(a).get.asInstanceOf[N1].eval[T1] shouldCloseTo (c.toStd mul b.toStd)
  }

  property("var0 * nonvar0 reverse node2") = forAll(genV0(), genNV0(), genN2()) { (a: Var0, b: N0, c: N2) =>
    val g = Mul00(a, b).reverse(c)
    g(a).get.asInstanceOf[N2].eval[T2] shouldCloseTo (c.toStd mul b.toStd)
  }

  property("nonvar0 * var0 reverse node0") = forAll(genNV0(), genV0(), genN0()) { (a: N0, b: Var0, c: N0) =>
    val g = Mul00(a, b).reverse(c)
    g(b).get.asInstanceOf[N0].eval[T0] shouldCloseTo a.toStd * c.toStd
  }
  ...
  property("nonvar0 * var0 reverse node2") = forAll(genNV0(), genV0(), genN2()) { (a: N0, b: Var0, c: N2) =>
    val g = Mul00(a, b).reverse(c)
    g(b).get.asInstanceOf[N2].eval[T2] shouldCloseTo (c.toStd mul a.toStd)
  }

  property("var0 * var0 reverse node0") = forAll(genV0(), genN0()) { (a: Var0, b: N0) =>
    val g = Mul00(a, a).reverse(b)
    g(a).get.asInstanceOf[N0].eval[T0] shouldCloseTo 2 * a.toStd * b.toStd
  }

  property("var0 * var0 reverse node1") = forAll(genV0(), genN1()) { (a: Var0, b: N1) =>
    val g = Mul00(a, a).reverse(b)
    g(a).get.asInstanceOf[N1].eval[T1] shouldCloseTo (b.toStd mul (2 * a.toStd))
  }

  property("var0 * var0 reverse node2") = forAll(genV0(), genN2()) { (a: Var0, b: N2) =>
    val g = Mul00(a, a).reverse(b)
    g(a).get.asInstanceOf[N2].eval[T2] shouldCloseTo (b.toStd mul (2 * a.toStd))
  }
