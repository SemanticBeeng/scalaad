...
object Cosh0Spec extends Properties("Cosh0") with UnaryOp0SpecBase {
  ...
  property("eval") = forAll(genN0()) { (a: N0) =>
    op(a).eval[T0] shouldCloseTo math.cosh(a.toStd)
  }

  property(s"${op("var0")} forward w.r.t self") = forAll(genV0()) { (a: Var0) =>
    op(a).forward[N0, N0](a).eval[T0] shouldCloseTo math.sinh(a.toStd)
  }
  ...
  property(s"${op("var0")} reverse node1") = forAll(genV0(), genN1()) { (a: Var0, b: N1) =>
    val g = op(a).reverse(b)
    g(a).get.asInstanceOf[N1].eval[T1] shouldCloseTo (b.toStd mul math.sinh(a.toStd))
  }

  property(s"${op("var0")} reverse node2") = forAll(genV0(), genN2()) { (a: Var0, b: N2) =>
    val g = op(a).reverse(b)
    g(a).get.asInstanceOf[N2].eval[T2] shouldCloseTo (b.toStd mul math.sinh(a.toStd))
  }
