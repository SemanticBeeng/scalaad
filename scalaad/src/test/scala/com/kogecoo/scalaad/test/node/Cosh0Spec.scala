...
object Cosh0Spec extends Properties("Cosh0") with NodeSpecBase {
  ...
  property("cosh(var0) reverse node0") = forAll(genV0(), genN0()) { (a: Var0, b: N0) =>
    val g = Cosh0(a).reverse(b)
    g(a).get.asInstanceOf[N0].eval[T0] shouldCloseTo b.toStd * math.sinh(a.toStd)
  }

  property("cosh(var0) reverse node1") = forAll(genV0(), genN1()) { (a: Var0, b: N1) =>
    val g = Cosh0(a).reverse(b)
    g(a).get.asInstanceOf[N1].eval[StdVec[T0]] shouldCloseTo (b.toStd mul math.sinh(a.toStd))
  }

  property("cosh(var0) reverse node2") = forAll(genV0(), genN2()) { (a: Var0, b: N2) =>
    val g = Cosh0(a).reverse(b)
    g(a).get.asInstanceOf[N2].eval[StdMat[T0]] shouldCloseTo (b.toStd mul math.sinh(a.toStd))
  }
