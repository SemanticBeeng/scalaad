...
object Sin0Spec extends Properties("Sin0") with UnaryOp0SpecBase {
  ...
  property("eval") = forAll(genN0()) { (a: N0) =>
    op(a).eval[T0] shouldCloseTo math.sin(a.toStd)
  }

  property(s"${op("var0")} forward w.r.t self") = forAll(genV0()) { (a: Var0) =>
    op(a).forward[N0, N0](a).eval[T0] shouldCloseTo math.cos(a.toStd)
  }
