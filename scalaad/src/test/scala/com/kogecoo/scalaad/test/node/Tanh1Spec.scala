...
object Tanh1Spec extends Properties("Tanh1") with UnaryOp1SpecBase {

  private[this] def deriv(x: Double): Double = 1 - math.tanh(x) * math.tanh(x)
  ...
  property("eval") = forAll(genN1()) { (a: N1) =>
    op(a).eval[T1] shouldCloseTo a.toStd.map(math.tanh)
  }
