...
object Sin1Spec extends Properties("Sin1") with UnaryOp1SpecBase {
  ...
  property("eval") = forAll(genN1()) { (a: N1) =>
    op(a).eval[T1] shouldCloseTo a.toStd.map(math.sin)
  }

  property(s"${op("var1")} forward w.r.t self") = forAll(genV1()) { (a: Var1) =>
    op(a).forward[N1, N2](a).eval[T2] shouldCloseTo diag(a.toStd.map(math.cos))
  }
