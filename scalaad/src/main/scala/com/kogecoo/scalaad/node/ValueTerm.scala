...
trait ValueTerm[S <: Shape]  extends Term[S]{
  val shape: S
  def forward[W, O](w: W)(implicit F: Forward[ValueTerm[S], W, O]): O = F.forward(this, w)
  def reverse[G](g: G)(implicit R: Reverse[ValueTerm[S], G]): Grad = R.reverse(this, g)

  def eval[V](implicit E: Eval[ValueTerm[S], V]): V = E.eval(this)
  def grad(implicit R: Reverse[ValueTerm[S], N0]): Grad = reverse[N0](One0())
}
