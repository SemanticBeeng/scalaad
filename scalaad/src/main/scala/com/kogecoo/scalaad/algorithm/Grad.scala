...
class Grad(val grad: Map[ValueTerm[_ <: Shape], ValueTerm[_ <: Shape]]) {
  ...
  def apply(node: ValueTerm[_ <: Shape]): Option[ValueTerm[_ <: Shape]] = grad.get(node)
...
object Grad {
  ...
  def apply(node: ValueTerm[_ <: S], grad: ValueTerm[_ <: S]): Grad = new Grad(Map(node -> grad))

  def empty: Grad = new Grad(Map.empty[ValueTerm[_ <: S], ValueTerm[_ <: S]])

  def where(cond: BooleanTerm[_ <: S], g1: Grad, g2: Grad): Grad = {
    val keys = g1.grad.keySet | g2.grad.keySet
    val (m: Map[ValueTerm[_ <: S], ValueTerm[_ <: S]]) = (for (k <- keys) yield {

      val value: Option[ValueTerm[_ <: S]] = (g1.grad.get(k), g2.grad.get(k)) match {
  ...
  private[this] def makeZero(zeroShape: S): ValueTerm[_ <: S] = zeroShape match {
    case s: S0 => Zero0()
    case s: S1 => Zero1(s)
    case s: S2 => Zero2(s)
  }

  private[this] def makeWhere(cond: BooleanTerm[_ <: S], n1: ValueTerm[_ <: S], n2: ValueTerm[_ <: S]): ValueTerm[_ <: S] = {
  ...
  implicit class GradOp(val self: Grad) extends AnyVal {
    def ++(other: Grad): Grad = {
      ...
      val (m: Map[ValueTerm[_ <: S], ValueTerm[_ <: S]]) = (for {
        k <- keys
        s =  self.grad.get(k)
        o =  other.grad.get(k)
    ...
    def add(g1: ValueTerm[_ <: S], g2: ValueTerm[_ <: S]): ValueTerm[_ <: S] = {
