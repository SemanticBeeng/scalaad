...
case class Mul11[V[_], T](lhs: N1[V, T], rhs: N1[V, T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } * ${ rhs.toString })"

  override def apply(): T = r.mul11(lhs(), rhs())
...
case class Mul22[M[_], T](lhs: N2[M, T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } * ${ rhs.toString })"

  override def apply(): M[T] = r.mul22(lhs(), rhs())
...
case class BroadcastMul01[V[_], T](lhs: N0[T], rhs: N1[V, T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul01(lhs(), rhs())
...
case class BroadcastMul10[V[_], T](lhs: N1[V, T], rhs: N0[T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul10(lhs(), rhs())
...
case class BroadcastMul12[M[_], V[_], T](lhs: N1[V, T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul12(lhs(), rhs())
...
case class BroadcastMul21[M[_], V[_], T](lhs: N2[M, T], rhs: N1[V, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul21(lhs(), rhs())
...
case class BroadcastMul02[M[_], T](lhs: N0[T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul02(lhs(), rhs())
...
case class BroadcastMul20[M[_], T](lhs: N2[M, T], rhs: N0[T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :* ${ rhs.toString })"

  override def apply(): T = r.mul20(lhs(), rhs())
