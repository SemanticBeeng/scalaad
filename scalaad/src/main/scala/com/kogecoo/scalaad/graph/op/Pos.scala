...
case class Pos0[T](v: N0[T]) extends N0[T] {
  val r = implicitly[ValueRule[U, T]]

  override def toString: String = s"+(${ v })"

  override def apply(): T = r.pos0(v)
...
case class Pos1[V[_], T](v: N1[V, T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"+(${ v })"

  override def apply(): V[T] = r.pos1(v)
...
case class Pos2[M[_], T](v: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"+(${ v })"

  override def apply(): M[T] = r.pos2(v)
