...
case class Sub11[V[_], T](lhs: N1[V, T], rhs: N1[V, T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } - ${ rhs.toString })"

  override def apply(): T = r.sub11(lhs(), rhs())
...
case class Sub22[M[_], T](lhs: N2[M, T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } - ${ rhs.toString })"

  override def apply(): M[T] = r.sub22(lhs(), rhs())
...
case class BroadcastSub01[V[_], T](lhs: N0[T], rhs: N1[V, T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub01(lhs(), rhs())
...
case class BroadcastSub10[V[_], T](lhs: N1[V, T], rhs: N0[T]) extends N1[V, T] {
  val r = implicitly[ValueRule[V, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub10(lhs(), rhs())
...
case class BroadcastSub12[M[_], V[_], T](lhs: N1[V, T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub12(lhs(), rhs())
...
case class BroadcastSub21[M[_], V[_], T](lhs: N2[M, T], rhs: N1[V, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub21(lhs(), rhs())
...
case class BroadcastSub02[M[_], T](lhs: N0[T], rhs: N2[M, T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub02(lhs(), rhs())
...
case class BroadcastSub20[M[_], T](lhs: N2[M, T], rhs: N0[T]) extends N2[M, T] {
  val r = implicitly[ValueRule[M, T]]

  override def toString: String = s"(${ lhs.toString } :- ${ rhs.toString })"

  override def apply(): T = r.sub20(lhs(), rhs())
